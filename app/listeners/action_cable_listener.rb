class ActionCableListener < BaseListener # rubocop:disable Metrics/ClassLength
  ...
  def assignee_changed(event)
    conversation, account = extract_conversation_and_account(event)
    tokens = conversation_user_tokens(account, conversation)
    tokens += previous_assignee_token(event)
  ...
  # CommMate: Filter broadcast tokens based on per-user conversation permissions.
  # Mirrors the logic in Conversations::PermissionFilterService so WebSocket
  # events match what each agent sees via the HTTP API.
  def conversation_user_tokens(account, conversation)
    Conversations::BroadcastTokenService.new(account: account, conversation: conversation).permitted_tokens
  end

  def previous_assignee_token(event)
    previous_assignee_id = event.data.dig(:changed_attributes, 'assignee_id')&.first
    return [] if previous_assignee_id.blank?

    user = User.find_by(id: previous_assignee_id)
    user ? [user.pubsub_token] : []
  end
