...
#
# Orchestrates the creation of a WhatsApp inbox backed by Evolution API.
#
# For Baileys:
...
#
# For Cloud API:
#   1. Creates Evolution instance with Chatwoot integration enabled (autoCreate:false)
#   2. Creates Chatwoot Channel::Api with webhook_url pointing to Evolution
#   3. Inbox is immediately usable (no QR needed)
#
# Usage:
...
#     channel: 'baileys',
#     meta_credentials: {} # or { token:, number:, business_id: } for cloud API
#   )
...
module EvolutionApi
  class InboxProvisioner
    ...
    attr_reader :account, :inbox_name, :channel, :meta_credentials, :instance_name

    def initialize(account:, inbox_name:, channel:, meta_credentials: {})
      @account = account
    ...
    def provision!
      ...
      configure_chatwoot_integration_for_cloud_api! if cloud_api?
    ...
    def find_or_create_integration_user
      ...
      User.create!(
        ...
        type: 'SuperAdmin' # Ensure it can access any account when added as admin
      )
    end
    ...
    def ensure_account_admin_membership!(user)
      ...
      if existing_membership.present?
        # Ensure they're an administrator
    ...
    def ensure_access_token!(user)
      # AccessToken is created automatically via AccessTokenable concern
    ...
    def create_evolution_instance!
      ...
      # For Cloud API, add Meta credentials
      if cloud_api?
        options.merge!(
          token: meta_credentials[:token],
          number: meta_credentials[:number],
          business_id: meta_credentials[:business_id]
        )
      end

      # For Baileys: Create instance WITHOUT any Chatwoot config
      # User will enable integration after scanning QR code
      # For Cloud API: Also create without Chatwoot config here,
      # we'll enable it separately via set_chatwoot_integration
    ...
    def configure_chatwoot_integration_for_cloud_api!
      # Only for Cloud API: Enable Chatwoot integration immediately
      # For Baileys, this is done after QR scan via the controller
      evolution_client.set_chatwoot_integration(
        ...
        chatwoot_config: {
          ...
          auto_create: false, # We create inbox ourselves
          sign_msg: true,
    ...
    def build_evolution_metadata
      metadata = {
        ...
        'evolution_channel' => channel_type_for_storage,
        'evolution_url' => evolution_api_url,
        'webhook_verify_token' => generate_webhook_verify_token
      }

      # Add Meta credentials reference for Cloud API (not the actual token)
      if cloud_api?
    ...
    def channel_type_for_storage
      channel == 'whatsapp_cloud_api' ? 'whatsapp_cloud_api' : 'baileys'
    end
    ...
    def cleanup_on_failure!
      # Attempt to delete the Evolution instance if it was created
