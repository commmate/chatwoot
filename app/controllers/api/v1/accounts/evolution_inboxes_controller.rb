...
#
# Handles Evolution API-backed WhatsApp inbox operations.
...
class Api::V1::Accounts::EvolutionInboxesController < Api::V1::Accounts::BaseController
  ...
  # Creates a new Evolution-backed WhatsApp inbox
  def create
    provisioner = EvolutionApi::InboxProvisioner.new(
      account: Current.account,
      inbox_name: provision_params[:inbox_name],
      channel: provision_params[:channel],
      meta_credentials: meta_credentials_params
    )
  ...
  def update_chatwoot_settings
    # Build config with fixed fields that can't be changed + user-provided fields
  ...
  # GET /api/v1/accounts/:account_id/evolution/inboxes/:inbox_id/templates
  # Lists all WhatsApp templates for Cloud API inboxes
  def templates
    validate_cloud_api_inbox!

    templates = evolution_client.find_templates(instance_name: evolution_instance_name)
    render json: { templates: templates }
  rescue EvolutionApi::Client::ApiError => e
    render json: { error: e.message }, status: :unprocessable_entity
  end

  # POST /api/v1/accounts/:account_id/evolution/inboxes/:inbox_id/templates
  # Creates a new WhatsApp template
  def create_template
    validate_cloud_api_inbox!

    result = evolution_client.create_template(
      instance_name: evolution_instance_name,
    ...
    render json: { error: e.message, details: e.response_body }, status: :unprocessable_entity
  end

  # PATCH /api/v1/accounts/:account_id/evolution/inboxes/:inbox_id/templates/:template_id
  # Updates an existing WhatsApp template
  def update_template
    validate_cloud_api_inbox!

    result = evolution_client.edit_template(
      instance_name: evolution_instance_name,
      template_data: template_update_params.merge(template_id: params[:template_id])
    )
  ...
  # DELETE /api/v1/accounts/:account_id/evolution/inboxes/:inbox_id/templates
  # Deletes a WhatsApp template by name
  def destroy_template
    validate_cloud_api_inbox!

    result = evolution_client.delete_template(
      instance_name: evolution_instance_name,
      template_name: params[:name],
      hsm_id: params[:hsm_id]
    )
  ...
  def validate_cloud_api_inbox!
    return if cloud_api_inbox?

    render json: { error: 'This operation is only available for WhatsApp Cloud API inboxes' }, status: :unprocessable_entity
  end

  def validate_baileys_inbox!
    return if baileys_inbox?

    render json: { error: 'This operation is only available for Baileys inboxes' }, status: :unprocessable_entity
  end
  ...
  def cloud_api_inbox?
    evolution_inbox? && evolution_channel == 'whatsapp_cloud_api'
  end

  def baileys_inbox?
    evolution_inbox? && evolution_channel == 'baileys'
  end
  ...
  def evolution_channel
    @inbox.channel.additional_attributes&.dig('evolution_channel')
  end
  ...
  def provision_params
    params.require(:evolution_inbox).permit(:inbox_name, :channel)
  end
  ...
  def instance_settings_params
    # Handle both root-level params and nested evolution_inbox params
  ...
  def build_chatwoot_update_config
    # Start with user-provided updatable fields
    ...
    config[:auto_create] = false # Already created

    config
  end

  def build_enable_integration_config
    # Full config for enabling integration after QR scan
    {
      url: chatwoot_reachable_url,
      account_id: Current.account.id,
      token: integration_user_token,
      name_inbox: @inbox.name,
      auto_create: false, # We already created the inbox
      enabled: true, # Explicitly enable the integration
      sign_msg: true,
  ...
  def chatwoot_reachable_url
    if Rails.env.development?
      ...
      if frontend_url&.include?('localhost') || frontend_url&.include?('127.0.0.1')
        # Try to get host IP for container → host communication
        host_ip = `ipconfig getifaddr en0 2>/dev/null`.strip
        host_ip = '192.168.0.22' if host_ip.blank? # fallback
        "http://#{host_ip}:3000"
      else
        frontend_url
      end
  ...
  def sanitize_chatwoot_settings(settings)
    # Remove sensitive token from response, keep other settings
  ...
  def format_error_message(error)
    # Extract the most useful error message from the exception
    message = error.message

    # Handle common error patterns
    if message.include?('Validation failed:')
      # Extract just the validation messages
