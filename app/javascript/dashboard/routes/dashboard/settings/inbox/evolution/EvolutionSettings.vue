...
export default {
  ...
  computed: {
    ...
    isCloudApi() {
      return this.evolutionChannel === 'whatsapp_cloud_api';
    },
    isConnected() {
    ...
    // Check if connected but integration not enabled (needs activation)
  ...
  mounted() {
    this.fetchSettings();
    // Start polling for connection state if Baileys and not connected
  ...
  methods: {
    async fetchSettings() {
      this.isLoading = true;
      try {
        const [settingsResponse, connectionResponse, instanceSettingsResponse] = await Promise.all([
        ...
        // If already connected on load, set flag to prevent alert on initial poll
        if (this.isConnected) {
    ...
    async saveInstanceSettings() {
      ...
      try {
        ...
        useAlert(this.$t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE_SAVE_SUCCESS'));
      } catch (error) {
        useAlert(
          error.response?.data?.error ||
    ...
    async connectInstance() {
      ...
      this.qrCode = { base64: null }; // Show loading state
      this.hasShownConnectedAlert = false; // Reset flag for new connection attempt
      ...
      try {
        ...
        if (this.isConnected) {
          ...
          if (!wasConnected && !this.hasShownConnectedAlert) {
            ...
            // Auto-enable integration if not enabled yet
    ...
    async openDisconnectDialog() {
      try {
        const confirmed = await this.$refs.disconnectConfirmDialog.showConfirmation();
        if (confirmed) {
    ...
    async confirmDisconnect() {
      ...
      try {
        ...
        this.hasShownConnectedAlert = false; // Reset flag for next connection
        useAlert(this.$t('INBOX_MGMT.EVOLUTION.SETTINGS.DISCONNECT_SUCCESS'));
        // Refresh connection state after logout
        await this.refreshConnection();
      } catch (error) {
...
<template>
  <div class="mx-8">
        ...
        <div class="flex flex-col gap-4">
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm font-medium text-n-slate-12">
              {{ $t(`INBOX_MGMT.EVOLUTION.SETTINGS.STATUS.${connectionStatus.toUpperCase()}`) }}
            </span>
          </div>

          <div class="text-sm text-n-slate-11">
            <p>
              <strong>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE_NAME') }}:</strong>
              {{ evolutionInstanceName }}
            </p>
            <p>
              <strong>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.CHANNEL_TYPE') }}:</strong>
              {{ isBaileys ? 'Baileys' : 'WhatsApp Cloud API' }}
            </p>
            <p>
              <strong>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INTEGRATION_STATUS') }}:</strong>
              {{ ' ' }}
              <span :class="integrationEnabled ? 'text-green-600' : 'text-red-600'">
                {{ integrationEnabled ? $t('INBOX_MGMT.EVOLUTION.SETTINGS.INTEGRATION_ACTIVE') : $t('INBOX_MGMT.EVOLUTION.SETTINGS.INTEGRATION_INACTIVE') }}
              </span>
            </p>
          </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <h4 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                  {{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INTEGRATION_NOT_ENABLED_TITLE') }}
                </h4>
                <p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                  {{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INTEGRATION_NOT_ENABLED_DESC') }}
                </p>
                <NextButton
                  class="mt-3"
                  solid
                  blue
                  :label="$t('INBOX_MGMT.EVOLUTION.SETTINGS.ENABLE_INTEGRATION_BUTTON')"
          ...
          <div v-if="isBaileys && !isConnected" class="mt-4">
            ...
            <!-- QR Code Display -->
            <div v-if="qrCode" class="mt-4 p-4 bg-white dark:bg-n-slate-2 rounded-lg">
      ...
      >
        <div class="flex flex-col gap-4">
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.REJECT_CALL') }}</span>
          </label>
          ...
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.GROUPS_IGNORE') }}</span>
          </label>
          <p class="text-xs text-n-slate-11 -mt-2 ml-6">
          ...
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.ALWAYS_ONLINE') }}</span>
          </label>
          <p class="text-xs text-n-slate-11 -mt-2 ml-6">
          ...
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.READ_MESSAGES') }}</span>
          </label>
          <p class="text-xs text-n-slate-11 -mt-2 ml-6">
          ...
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.SYNC_FULL_HISTORY') }}</span>
          </label>
          <p class="text-xs text-n-slate-11 -mt-2 ml-6">
            {{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.SYNC_FULL_HISTORY_DESC') }}
          </p>

          <label class="flex items-center gap-2">
            <input v-model="readStatus" type="checkbox" />
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.INSTANCE.READ_STATUS') }}</span>
          </label>
          <p class="text-xs text-n-slate-11 -mt-2 ml-6">
      ...
      >
        <div class="flex flex-col gap-4">
          ...
          <label class="flex items-center gap-2">
            ...
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.REOPEN_CONVERSATION') }}</span>
          </label>

          <label class="flex items-center gap-2">
            <input v-model="conversationPending" type="checkbox" />
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.CONVERSATION_PENDING') }}</span>
          </label>

          <label class="flex items-center gap-2">
            <input v-model="mergeBrazilContacts" type="checkbox" />
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.MERGE_BRAZIL_CONTACTS') }}</span>
          </label>

          <label class="flex items-center gap-2">
            <input v-model="importContacts" type="checkbox" />
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.IMPORT_CONTACTS') }}</span>
          </label>

          <label class="flex items-center gap-2">
            <input v-model="importMessages" type="checkbox" />
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.IMPORT_MESSAGES') }}</span>
          </label>

          <label v-if="importMessages" class="flex flex-col gap-1">
            <span>{{ $t('INBOX_MGMT.EVOLUTION.SETTINGS.DAYS_LIMIT_IMPORT') }}</span>
            <input
    ...
    <ConfirmationModal
      ...
      :description="$t('INBOX_MGMT.EVOLUTION.SETTINGS.DISCONNECT_CONFIRM_MESSAGE')"
