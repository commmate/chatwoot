<template>
  <div v-if="showPipelinesSection" class="conversation-pipelines bg-slate-25 dark:bg-slate-900 rounded-lg p-4 mb-4">
    <!-- Section Header -->
    ...
    <div v-if="pipelineFields.length > 0" class="space-y-2">
      ...
      >
        ...
        <select
          ...
          @change="updatePipelineStage(pipelineField.key, $event.target.value)"
    ...
    <!-- Empty State -->
    <div v-else class="text-sm text-slate-500 dark:text-slate-400 text-center py-4">
...
const pipelineFields = computed(() => {
  ...
  Object.entries(customAttributes).forEach(([key, value]) => {
    if (pipelineFieldKeys.includes(key)) {
      const pipeline = accountPipelines.value.find(p => p.custom_field_key === key);
      if (pipeline) {
        pipelines.push({
          key,
          value,
